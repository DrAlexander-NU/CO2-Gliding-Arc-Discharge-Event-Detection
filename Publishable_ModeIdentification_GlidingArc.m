...
for i = 1:numFiles
    ...
    if is_low_enough
        ...
        elseif (k - drop_check_start_idx + 1) >= drop_check_length
            % This is the N-th consecutive peak below the drop threshold! Run END.
...
blue_marker_times = [];   % SEPARATE storage from green marker data (voltage drop only)
...
if do_export
    ...
    %Attempting to export full voltage/current signal in a single excel
    %will oversaturate the document and fail; must export in sections
    ...
    blue_export = blue_in_window * 1000;   % ms
    ...
    green_export = green_in_window * 1000;   % ms
